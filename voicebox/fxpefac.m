...
%fres = 1.81 * fs / 512;            % bandwidth of initial spectrogram (Hz)
%[tx,f,MIX]=spgrambw(s,fs,fres,[fmin fstep fmax],[],p.tinc);
winlen2 = 1.81 * fs / fres;
[~, f, tx, MIX]=spectrogram(s, hamming(winlen2), winlen2 - p.tinc * fs, pow2(nextpow2(4*winlen2)), fs);
nframes=length(tx);
